...
| 信念演化 | 信念演化 | APPEND（`时间\|触发\|旧→新`） | 变质判定留痕/管道B（均为信念演化写入时机·不涉及升级） |
...
```
###META: 压力扫描 人际✓/增殖✓/轨道✓/跨叙事✓   ← 静默自查锚点（每轮必写·四信号逐项勾·解析忽略·不落盘不展示）
...
>
> **`###META:` 行（2026-08-09 加入）：** 静默自查锚点——批次内元数据行，解析器忽略（不落盘、不产生写入 ops），用于四信号自检等执行留痕；audit/dry-run 输出可显示但不会因内容报错。格式自由，首行为 `###META: 标签 内容`。
