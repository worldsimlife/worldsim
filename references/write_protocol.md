...
> 与焦外 CT 互补：CT=冲突视角（关系/节拍/代价），pending_actions=事件视角（行动/状态/时段/结果/预计影响/揭示场景）·各写各的侧重。
...
>
> **`###BEATSHEET:` 行（2026-08-14 加入）：** 事件线动作声明——完整推进轮必写，解析器识别为批次级元数据（不落盘、不产生写入 ops、不打断当前字段）。取值五选一：`add` / `stay N` / `advance N 拍名` / `rewrite N` / `clear N`。阶段3 按它先执行 `beatsheet` 命令，再执行 write-raw；缺失时 `gate dramatist --check` 对完整推进轮硬性拦截。
...
**Windows 平台（PowerShell·UTF-8 中文经控制台 GBK 码页被破坏·写入内容变 `?`）：** 把批次/叙事写入 UTF-8 临时文件，再用 `cmd /c "python3 {skill_dir}/scripts/worldctl.py {世界名} write-raw --batch < 临时文件"`（write_narrative.py 同理）重定向喂 stdin——管道与重定向需在 `cmd /c` 内执行才能正确保留 UTF-8 字节。
...
>
> **`--force`（显式回退专用·仅 `--batch`）：** 回退手工重建（无快照）时追加——`write-raw --batch --force` 绕过 audit ④ 轮次单调（轮次可回退）与 ⑬b 反应轨迹覆盖写（可覆盖裁剪）；其余硬性检查（角色反应四件套/载体/scene_state 落点/记忆留痕）照常拦截。非幂等同前（同一批次只执行一次·验证用 `--dry-run`/read/validate）；回退后必做残留扫描 + validate（见 references/rollback.md）。
