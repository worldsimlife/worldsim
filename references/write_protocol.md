...
| 节拍表 | conflicts 顶层 `节拍表`（编号事件线 `节拍表.{N}`：事件线/当前拍/拍序） | 脚本维护 `worldctl.py <世界> beatsheet`（show/add/advance/rewrite/clear）·LLM 不直接改 | 建线=全新事件线（beatsheet add·N 递增）·收敛 advance 写当前拍=本轮实际到达·换线=现实不承接→rewrite·清线=当前拍=余波→clear |
