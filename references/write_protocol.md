...
**audit 语义不变量（对应闸门中可代码化的部分）：** 硬性（写入时单字段顶回）——① 行动卡三件套（`###ACTION:` 行 驱动/情绪/强度 缺一拒绝）+ 有条件代价（关联CT∈①推进池且施压方向∈四爆破或当前拍戏剧问题∈资源/关系/控制权/认知类时`代价:`必填且后非空·其余过渡豁免软警·④角色批）；② 被争夺资源必须含 `当前载体=`/`当前持有者=`；④ `world_state.轮次` 单调递增；⑤ `scene_state` 落点必须有焦点场景目录——**落点校验在执行路径（write-raw 写入时）强制**；独立 audit 预检与 gate 仅软提示（场景目录由启动序列入场物化（初始场景）/⑤场记 init_scene（切换场景）创建·先于批次写入·见 session_recovery.md 第二章 / scene_management.md §场景切换流程）。软性（不拦截·validate 汇总）——③ 记忆锚点单条 ≤100 字、写入后总量 ≤3000。
...
> **运行时合并（谓词分派）：** 单批多段（批次内重复出现 `###STAGE:` 行即切段，audit 与闸门逐段独立执行，检查项/写入矩阵/单字段顶回语义均不变）。**单射批段完整性（硬性·防阶段整段缺席）：** 合并批（≥2 段·非 `--resume-from` 重提·非 `--force` 回退）必须五段齐备（戏剧家/编剧/导演/角色/场记·轻=最小维护批仍须出段）——缺段=`[BATCH-FAIL]` 零副作用整体拦截（作家段不在合并批·经 write_narrative 独立落盘）；串行路径与无 `###STAGE` 批不受影响。合并批中途失败＝失败段之前的段已生效；脚本会在 stderr 输出 `[BATCH-FAIL]` 对账报告——**已完整落盘段清单 + 已落盘结构动作清单（重提时必须剔除的 `###STORYLINE/###BEAT`）+ 建议 `--resume-from <失败段号>`**。修正后：按报告剔除已落盘结构动作 → 用 `--resume-from` 只重提失败段及其后段 → 先 `--dry-run` 预演（剔除段应显示 `[无变化]` 且无 `[SKIP]` 误报）→ 再落盘。
...
> **批次级元数据行（不产生写入 ops·不落盘）**：`###STAGE:` 阶段声明｜`###META:` 静默自查锚点｜`###STORYLINE:` 结构动作（add/rewrite 后跟事件线 YAML 块·②编剧·写 storylines）｜`###BEAT:` 演出指针动作（set/stay/advance·③导演·写 direction）｜`###ACTION:` 行动卡（四件套+耗时·④角色·audit ①/①b/⑨ 检查对象）｜`###SCHEDULE:` 行动链留痕（④角色·不落盘·不裁决调度——调度权威=③ direction.调度单）。`###STORYLINE/###BEAT` 由 write-raw 自动执行对应子命令（失败=批次拦截 exit 1·LLM 不手动调用）。
...
> 行动卡三件套（驱动/情绪/强度·###ACTION 行）+ 有条件代价（见上条）由 audit ①/①b/⑨ 检查（④角色批）；`代价:` 有值时为标准模式回复正文「本轮代价」行来源（沉浸模式该行不输出）。
...
| 连续行动轨迹 | 连续行动轨迹 | APPEND 追加本轮（④角色·七子字段·近 10 轮滚动·APPEND 后脚本自动删最旧·LLM 零操作；**覆盖写须保留旧值首末轮次标记**·audit ⑬b 硬性拦截；增长控制=脚本滚动+validate 字数告警兜底） | 角色本轮有行动 |
...
>
> **CHAR_* key 校验：** `###FILE: CHAR_*` 必须命中已存在的角色 state 文件；空格/下划线混用或缺 `_state` 后缀但能唯一对应已有文件时，映射到该真实文件并给出 `[WARN]`。简写、缺字、多字或其他未命中 key **不自动猜测、不新建文件**：正式写入与 dry-run 均返回 `[ERR]`，同时列出已存在的可能 `CHAR_<全名>_state` key；请改用候选全名后重提批次。新角色先建 `characters/CHAR_<全名>.md` 并完成 `init-states`，再写入其 state。
