...
> **`###BEATSHEET:` 批次级元数据（事件线动作·完整推进轮必写）：** 与 `###META:` 一样不产生写入 op，用于声明本轮节拍表生命周期动作——`###BEATSHEET: add` / `###BEATSHEET: stay N` / `###BEATSHEET: advance N 拍名` / `###BEATSHEET: rewrite N` / `###BEATSHEET: clear N`。**write-raw --batch 解析该行自动执行对应子命令落盘节拍表**（`add`/`rewrite` 后直接跟事件线 YAML 块直到下一个 `###` 行·LLM 不手动调用·失败=批次拦截 exit 1）；CT 的 `当前节拍.拍名` 必须镜像为 `{N}-{拍名}`（N=事件线编号）。**节拍动作**：`stay N`=停留当前拍（当前拍内容未完成·拍序保持原样·默认动作）；`advance N 余波 形态:XXX`=顶点出线（受 `gate dramatist --check` 核验——形态∈四形态枚举+声明形态对应字段族至少一字段实质变化（≠顶点落点.基准值·收束证明）·缺形态/非法形态/字段不匹配/基准缺失/落点缺角色内部变量/双方全无变化 exit 1=撤回阶段1·见 references/phase_dramatist.md「顶点」条）；add/rewrite 时顶点拍缺 `顶点落点`（角色/内部变量）脚本当场拒绝（exit 1）。
...
| 下一个节拍(推荐) | CT-XX.下一个节拍(推荐) | KEY 覆盖 | 推进轮必写·多候选（/分隔）·方向灵活·用户指令可改 |
| 节拍表 | conflicts 顶层 `节拍表`（编号事件线 `节拍表.{N}`：事件线/当前拍/拍序·顶点拍含 顶点落点） | 脚本维护 `worldctl.py <世界> beatsheet`（show/add/stay/advance/rewrite/clear）·LLM 不直接改 | **每轮完整推进按序判定选一（承接判定先于完成度判定）**：余波→clear（不用等余波事件完成·清后建）·空表/新线→add（顶点拍预填 顶点落点·戏剧目标声明·缺则拒绝）·现实不承接→**默认 clear N 清线**（低阻力出口·复用余波清线语义·新内核可清后建）·判线仍有继续价值→rewrite N 重规划（当前拍按现实落位·顶点拍同步重填 顶点落点·缺则拒绝）·**承接不符时 advance 不参与判定**·承接成立后：内容未完成→stay N（拍序保持原样·默认动作）·内容已完成→advance N 下一拍（顶点=advance N 余波 形态:XXX·受 gate 收束核验）；以 `###BEATSHEET:` 声明（write-raw 自动执行） |
...
```
###META: 压力扫描 人际✓/增殖✓/轨道✓/跨叙事✓/记忆✓ Guest:3·未达 Angela:0·未达   ← 静默自查锚点（每轮必写·记忆✓ 后跟逐角色同类计数留痕·每出场角色必列·audit 机械对照批次 CHAR_state 角色集·缺角色=硬性拦截·`已触发`须同批含该角色信念演化 APPEND）
###BEATSHEET: advance 1 接触   ← 事件线动作（完整推进轮必写·add/rewrite 后跟事件线 YAML 块·参数在行内；当前拍未完成=stay N·顶点出线=advance N 余波 形态:XXX（收束证明·受 gate 核验）·由 write-raw 自动执行）
