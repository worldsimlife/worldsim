...
**临时文件约定：** 批次与叙事默认 stdin 直通（heredoc·`'EOF'` 免转义），不落临时文件；check 重提/调试场景确需落临时文件 → 默认系统临时目录（$TMPDIR·OS 自清理），系统 tmp 不可用时放 `worlds/{世界名}/tmp/`（命名 `cs_r{轮次}.txt` / `narrative_r{轮次}.txt`·用后即删·跨会话恢复时由加载序列执行 `tmp-clean` 清理）。
