...
**audit 语义不变量（对应闸门中可代码化的部分）：** 硬性（写入时单字段顶回）——① 角色反应四件套（`驱动:`/`情绪:`/`强度:`/`代价:` 缺一拒绝，`代价:` 后为空拒绝）；② 被争夺资源必须含 `当前载体=`/`当前持有者=`；④ `world_state.轮次` 单调递增；⑤ `scene_state` 落点必须有焦点场景目录——**落点校验在执行路径（write-raw 写入时）强制**；独立 audit 预检与 gate 仅软提示（场景目录由启动序列入场物化（初始场景）/场记阶段3 init_scene（切换场景）创建·先于批次写入·见 session_recovery.md 第二章 / scene_management.md §场景切换流程）。软性（不拦截·validate 汇总）——③ 记忆锚点单条 ≤100 字、写入后总量 ≤3000。
...
> **`###BEATSHEET:` 批次级元数据（事件线动作·完整推进轮必写）：** 与 `###META:` 一样不产生写入 op，用于声明本轮节拍表生命周期动作——`###BEATSHEET: add` / `###BEATSHEET: stay N` / `###BEATSHEET: advance N 拍名` / `###BEATSHEET: rewrite N` / `###BEATSHEET: clear N`。**write-raw --batch 解析该行自动执行对应子命令落盘节拍表**（`add`/`rewrite` 后直接跟事件线 YAML 块直到下一个 `###` 行·LLM 不手动调用·失败=批次拦截 exit 1）；CT 的 `当前节拍.拍名` 必须镜像为 `{N}-{拍名}`（N=事件线编号）。**节拍动作**：`stay N`=停留当前拍（**本拍戏剧问题未被当前冲突兑现·随批写明本轮兑现进展（谁被逼到新位置/控制权转移/退路减少）·无兑现进展=当前轮设计作废·退回重做**）；`advance N 余波`=顶点出线（受 `gate dramatist --check` 核验——顶点落点齐备+基准值存在+冲突双方关键状态有实质变化（收束证明底线）·形态可带可不带（落盘一致性参考·不参与硬性判定）·缺顶点落点/基准缺失/双方全无变化 exit 1=撤回阶段1·见 references/phase_dramatist.md「顶点」条）；add/rewrite 时顶点拍缺 `顶点落点`（角色/内部变量·用户角色为落点时必须同时预填 ≥1 个 NPC 爆破项）脚本当场拒绝（exit 1）。
...
| 项 | 键路径 | 写语义 | 触发 |
|----|--------|--------|------|
| CT 推进 | CT-XX.当前节拍.{拍名/关系状态/内部状态/角色反应/相位} | KEY 覆盖 | 每轮至少一条（D1 硬性） |
| 下一个节拍(推荐) | CT-XX.下一个节拍(推荐) | KEY 覆盖 | 推进轮必写·本轮结束时仍悬而未决的戏剧问题+冲突结构收敛方向（禁写角色推荐行动）·用户指令可改 |
| 节拍表 | conflicts 顶层 `节拍表`（编号事件线 `节拍表.{N}`：事件线/当前拍/拍序·顶点拍含 顶点落点） | 脚本维护 `worldctl.py <世界> beatsheet`（show/add/stay/advance/rewrite/clear）·LLM 不直接改 | **承接判定在④规划·推进判定在⑦收敛（行动结果后回判）**：余波→clear（不用等余波事件完成·清后建）·空表/新线→add（顶点拍预填 顶点落点·戏剧目标声明·含 NPC爆破·缺则拒绝）·现实不承接→**默认 clear N 清线**（低阻力出口·复用余波清线语义·新内核可清后建）·判线仍有继续价值→rewrite N 重规划（当前拍按现实落位·顶点拍同步重填 顶点落点·缺则拒绝）·**承接不符时 advance 不参与判定**·承接成立后（⑦）：本拍戏剧问题未兑现且本轮行动有兑现进展→stay N（拍序保持原样·无进展=重做）·问题已兑现→advance N 下一拍（顶点=advance N 余波·出线受 gate 收束核验）；以 `###BEATSHEET:` 声明（write-raw 自动执行） |
...
| 防御重构 | 防御形态 / 崩溃表现 / 防御有效性 | KEY 覆盖（防御形态/崩溃表现：重构后的新防御·空=档案默认） | 重构事件（三条件齐备：威胁源解除+支持性关系介入+信念重构·戏剧家阶段1判定·防御有效性升一档·见 references/phase_dramatist.md §记忆维护） |
...
```
###META: 压力扫描 人际✓/增殖✓/轨道✓/跨叙事✓/记忆✓ Guest:3·未达 Angela:0·未达 角色覆盖: Guest=更新,Angela=更新   ← 静默自查锚点（每轮必写·记忆✓ 后跟逐角色同类计数留痕·角色覆盖逐一标记 CT 反应角色·更新者必须有 CHAR_state·无变化须附理由·audit 差集硬拦截）
###BEATSHEET: stay 2   ← 事件线动作（完整推进轮必写·add/rewrite 后跟事件线 YAML 块·参数在行内；本拍问题未兑现且有进展=stay N·顶点出线=advance N 余波（形态可带可不带·受 gate 核验）·由 write-raw 自动执行）
...
- 轮次: 25
  ...
  档位: 脚本档
###APPEND: 信念演化        ← 条件：变质判定留痕/管道B（同类认知连续3条·入锚写前步骤③）
- 轮次: 26
  时间: 第1日 13:30-13:35
  触发事件: 我……
  旧→新: 旧信念…… → 新信念……
###KEY: 人际动态          ← 条件：管道A（锚点涉及特定对象且有实质变化·入锚写前步骤④）
Guest: 中立——他指出了父亲话里的漏洞
###FILE: scene_state
###APPEND: 场景时间线
· HH:MM-HH:MM 事件……
...
>
> **`###BEATSHEET:` 行（2026-08-14 加入）：** 事件线动作声明——完整推进轮必写，解析器识别为批次级元数据（不落盘、不产生写入 ops、不打断当前字段）。取值四选一：`add` / `advance N 拍名` / `rewrite N` / `clear N`。阶段3 按它先执行 `beatsheet` 命令，再执行 write-raw；缺失时 `gate dramatist --check` 对完整推进轮硬性拦截。
...
**Windows 平台（PowerShell·UTF-8 中文经控制台 GBK 码页被破坏·写入内容变 `?`）：** 把批次/叙事写入 UTF-8 临时文件，再用 `cmd /c "python3 {skill_dir}/scripts/worldctl.py {世界名} write-raw --batch < 临时文件"`（write_narrative.sh 同理）重定向喂 stdin——管道与重定向需在 `cmd /c` 内执行才能正确保留 UTF-8 字节。

- **落点：** 临时文件一律写系统临时目录（`$TMPDIR`·OS 自清理）；系统 tmp 不可用时才放 `worlds/{世界名}/tmp/`
- **命名：** `cs_r{轮次}.txt`（批次）/ `narrative_r{轮次}.txt`（叙事）·用后即删
- **清理：** 世界级 tmp/ 由跨会话恢复时 `worldctl.py <世界> tmp-clean` 清理；系统级由 OS 自清理
...
**内容行内嵌标记（硬性错误）：** `###FILE:`/`###KEY:`/`###APPEND:`/`###BEATSHEET:` 必须独占行首。内容行内出现这些标记（不在行首）= 上一字段内容与标记拼接（如脚本替换缺换行）→ audit 报「内容行内嵌标记」，该字段拒绝写入。**用脚本修改 change set 后必须过 audit 再落盘。**
