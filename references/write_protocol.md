...
> **`###BEATSHEET:` 批次级元数据（事件线动作·完整推进轮必写）：** 与 `###META:` 一样不产生写入 op，用于声明本轮节拍表生命周期动作——`###BEATSHEET: add` / `###BEATSHEET: advance N 拍名` / `###BEATSHEET: rewrite N` / `###BEATSHEET: clear N`。阶段3 在 write-raw 前调用 `worldctl.py <世界> beatsheet` 对应子命令落盘；CT 的 `当前节拍.拍名` 必须镜像为 `{N}-{拍名}`（N=事件线编号）。
...
| 节拍表 | conflicts 顶层 `节拍表`（编号事件线 `节拍表.{N}`：事件线/当前拍/拍序） | 脚本维护 `worldctl.py <世界> beatsheet`（show/add/advance/rewrite/clear）·LLM 不直接改 | **每轮完整推进必选一**：空表/新内核→add·当前拍未完→advance N 拍名·现实不承接→rewrite N·当前拍=余波→clear；以 `###BEATSHEET:` 声明并在 write-raw 前执行 |
