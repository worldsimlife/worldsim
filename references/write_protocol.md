...
> **运行时优化（可选·默认不用）：** 相邻阶段可合并为单批多段——批次内重复出现 `###STAGE:` 行即切段，audit 与闸门逐段独立执行（检查项/写入矩阵/单字段顶回语义均不变）。合并批中途失败＝失败段之前的段已生效；脚本会在 stderr 输出 `[BATCH-FAIL]` 对账报告——**已完整落盘段清单 + 已落盘结构动作清单（重提时必须剔除的 `###STORYLINE/###BEAT`）+ 建议 `--resume-from <失败段号>`**。修正后：按报告剔除已落盘结构动作 → 用 `--resume-from` 只重提失败段及其后段 → 先 `--dry-run` 预演（剔除段应显示 `[无变化]` 且无 `[SKIP]` 误报）→ 再落盘。
