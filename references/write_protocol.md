...
> **批次级元数据行（不产生写入 ops·不落盘）**：`###STAGE:` 阶段声明｜`###META:` 静默自查锚点｜`###STORYLINE:` 结构动作（add/rewrite 后跟事件线 YAML 块·②编剧·写 storylines）｜`###BEAT:` 演出指针动作（set/deepen/advance·③导演·写 direction）｜`###ACTION:` 行动卡（四件套+耗时·④角色·audit ①/①b/⑨ 检查对象）｜`###SCHEDULE:` 行动链留痕（④角色·不落盘·不裁决调度——调度权威=③ direction.调度单）。`###STORYLINE/###BEAT` 由 write-raw 自动执行对应子命令（失败=批次拦截 exit 1·LLM 不手动调用）。
