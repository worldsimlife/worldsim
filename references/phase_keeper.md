...
2. **焦外记录（pending_actions·场景级·缺文件按模板建·templates/pending_actions.yaml）**：③调度单.焦外（含循环预设）+ ④焦外角色状态 → 筛相关（冲突对抗方/时间线节点临近/揭示候选/用户关注）→ 记 `活跃中.{PA-ID}`（角色/时段/行动/当前状态/预计影响/揭示场景·标可揭示）；揭示（焦外→焦内）→ 移 `已完成`·记揭示场景。**焦外→焦内向局（每轮归位）**：调度单 `焦外→焦内` 层角色本轮须落到 `焦内活跃` 或 `背景` 最终桶（完成状态更新·gate director 机械核验），该层仅作过渡标注·不留下无人消费的中间态。
...
- **唯一通道=worldctl 脚本子命令**（write-raw --batch 等）；中文一律 stdin；非幂等（APPEND 重复执行会重复追加·同一批次只执行一次·验证用 read/validate/--dry-run·禁重放 write）——**例外=批次执行失败后的重提**（`--resume-from <本段号>`·非重放验证·部分落盘时轮次已=新值自动放行·见 write_protocol.md「部分落盘重提」）。
