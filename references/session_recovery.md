...
**触发：** 世界刚创建 或 reset_world.sh 重置后，用户说「启动/继续/恢复世界」类词。**前置：须先经 references/disclosures.md「进入确认」（声明+询问·会话内一次），用户确认后才执行本章加载序列；未确认不加载。** **写入提示：** 本章含真实写入——物化缺失动态文件（states/ 下 yaml）、tmp-clean 清理；全部属进入确认已声明的读写契约。
...
**触发：** ① 世界已存在时用户要求进入该世界（输入须含「启动/继续/恢复世界」并明确指向某世界名，**不含则不触发**）。**前置：须先经 references/disclosures.md「进入确认」（声明+询问·会话内一次），用户确认后才执行本章加载序列；未确认不加载。**（引擎代际替换豁免——会话内已确认过·见下）**写入提示：** 本章含真实写入——缺失动态文件物化、validate 修复、周期重置触发（4.6②）、知情边界清理（4.5）、tmp-clean（4.7）；全部属进入确认已声明的读写契约。**意图判定（硬性）：判定模糊（不确定用户是否要进入模拟）→ 询问用户确认，不默认按恢复处理**；用户确认恢复后：加载→校验→描绘→停住，不推进剧情；用户随后明确要求推进时再进入每轮流程。② **引擎代际替换**（对话进行中·引擎失效换新）——加载序列照走（**不重复进入确认**——会话内已确认过），但**不停住不描绘**：按原用户指令直接进入完整推进（代际替换由编排者指令声明，不依赖世界文件标记）。
...
4.7 **临时文件清理：** `worldctl.py <世界> tmp-clean`——清理上次会话遗留的世界 `tmp/` 目录（过程临时文件·批次/叙事草稿用后即删）
