...
**触发：** ① 新会话/断线恢复后的第一条输入——如「继续世界」「恢复」「开始」；或世界已存在时用户要求进入该世界。判定失败时：优先按恢复处理（加载→校验→描绘→停住），不推进剧情；用户随后明确要求推进时再进入每轮流程。② **引擎代际替换**（对话进行中·引擎失效换新）——加载序列照走，但**不停住不描绘**：按原用户指令直接进入完整推进（代际替换由编排者指令声明，不依赖世界文件标记）。
