...
> 角色卡（character card）是 SillyTavern / Chub.ai 生态的角色交换格式：一张 PNG 图片内嵌角色的全部设定 JSON。本参考说明如何把外部角色卡导入 WorldSim 世界：**脚本机械提取全部字段 → LLM 综合生成正式角色档案**（无草稿、无「待戏剧家精炼」环节——CHAR.md 生成后运行中不修改，综合生成一次到位）。
...
1. **脚本机械提取**：`import_card.py` 解析 PNG/JSON → 归一化 V1/V2/V3 → 把**全部内容**（所有字段含嵌套结构，一个不丢）完整留存 `{世界}/import/{名}.card.json`，并打印结构化摘要。脚本不做任何理解——那是 LLM 的能力，不用死代码代替。**披露：** 角色卡全文（含问候语、世界观、潜在敏感/版权内容）会原样存档于本地磁盘；导入前请确认你有权使用该卡内容，导入后删除 `{世界}/import/{名}.card.json` 即可移除原文。
2. **LLM 内容驱动综合**：读取素材**全部内容**，形成对角色/卡的整体理解，再按 `templates/CHAR_.md` 语义结构填充正式档案：
...
| 知识库/世界书（character_book） | 小体量 → 背景·知识条目；大体量（数十条目） → 补充设定·索引引用 + card.json 全文 | |
| 喜好/厌恶/痛恨/怪癖/作者设定 | 补充设定区 | CHAR 无对应字段——收容不丢 |
| 剧情分支/场景清单（creator_notes 中） | 补充设定区 + 情景参考 | 高价值剧情素材，吸收 |
| system_prompt / post_history_instructions | **不导入** | 与 WorldSim 引擎语义冲突（WorldSim 的冲突/循环/记忆机制由引擎自身驱动，禁止用角色卡系统提示词覆盖） |

素材文件统一存 `{世界}/import/{名}.card.json`（含完整原始字段 + `_import_notes` 说明）。
...
- **导入目录**：`{世界}/import/` 已 gitignore，素材不污染 `validate` / `scan` / 快照。
