...
1. **脚本机械提取**：`import_card.py` 解析 PNG/JSON → 归一化 V1/V2/V3 → 把内容留存 `{世界}/import/{名}.card.json`，并打印结构化摘要。脚本不做任何理解——那是 LLM 的能力，不用死代码代替。
2. **披露**：处理前向用户披露要求用户确认：导入即表示同意该处理与存档（导入全部内容并发送LLM处理，本地进行存档）。导入前请确认你有权使用该卡内容、且不含你不想披露给 LLM 的敏感个人信息；删除 `{世界}/import/{名}.card.json` 即可移除原文。如角色卡内容含潜在敏感/版权/NSFW/提示注入等内容，需要特别提醒用户确认同意。
